    ...
    def __call__(self, results):
        ...
        if length >= self.crop_size:
            ...
            if 'contact' in results:
                ...
                contact = contact[idx:idx + self.crop_size]
            ...
            if 'contact' in results:
                contact = results['contact']
                D1= contact.shape[1:]
                padding_contact_zeros = np.zeros((padding_length, *D1), dtype=np.float32)
                contact = np.concatenate([contact, padding_contact_zeros], axis=0)
