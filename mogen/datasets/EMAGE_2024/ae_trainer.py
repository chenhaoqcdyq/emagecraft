...
from mogen.datasets.EMAGE_2024 import train_mix
# torch.autograd.set_detect_anomaly(True)

class CustomTrainer(train_mix.BaseTrainer):
    """
    ...
    def __init__(self, args):
        ...
        self.joint_mask = self.train_data.joint_mask
    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(self.train_loader):
            tar_pose = dict_data["pose"][..., :-4]
            # tar_contact = dict_data["pose"][:, -4:]
            pose_each_file = tar_pose * self.joint_mask
            tar_pose = pose_each_file[..., self.joint_mask.astype(bool)]
            # tar_pose = torch.cat((pose_each_file, tar_contact), dim=-1)
            tar_beta = dict_data["beta"].to(self.rank).float()
            tar_trans = dict_data["trans"].to(self.rank).float()
            tar_pose = tar_pose.to(self.rank).float()
            bs, n, j = tar_pose.shape[0], tar_pose.shape[1], self.joints
            tar_exps = torch.zeros((bs, n, 100)).to(self.rank)
            ...
            g_loss_final = g_loss_final + loss_rec
            ...
            g_loss_final = g_loss_final + velocity_loss 
            g_loss_final = g_loss_final + acceleration_loss 
             # vertices loss
            if self.args.rec_ver_weight > 0:
                ...
                num_batches = (bs + batch_size - 1) // batch_size
            
                vertices_rec_tensor, vertices_tar_tensor = [], []
                for i in range(num_batches):
                    start_idx = i * batch_size
                    end_idx = min((i + 1) * batch_size, bs)
                    # 获取当前批次的数据
                    tar_pose_batch = tar_pose.reshape(bs, n, -1)[start_idx:end_idx]
                    tar_pose_batch = tar_pose_batch.reshape(-1, tar_pose.shape[-1])
                    # tar_pose_batch = tar_pose[start_idx:end_idx]
                    rec_pose_batch = rec_pose.reshape(bs, n, -1)[start_idx:end_idx]
                    rec_pose_batch = rec_pose_batch.reshape(-1, rec_pose.shape[-1])
                    # rec_pose_batch = rec_pose[start_idx:end_idx]
                    tar_beta_batch = tar_beta[start_idx:end_idx]
                    tar_trans_batch = tar_trans[start_idx:end_idx]
                    tar_exps_batch = tar_exps[start_idx:end_idx]

                    # 计算 rec_pose 对应的 joints
                    vertices_rec = self.smplx(
                        betas=tar_beta_batch.reshape(-1, 300),  # 当前批次的 betas
                        transl=tar_trans_batch.reshape(-1, 3) - tar_trans_batch.reshape(-1, 3),  # 当前批次的 transl
                        expression=tar_exps_batch.reshape(-1, 100),  # 当前批次的 expressions
                        jaw_pose=rec_pose_batch[:, 66:69],  # 当前批次的 jaw_pose
                        global_orient=rec_pose_batch[:, :3],  # 当前批次的 global_orient
                        body_pose=rec_pose_batch[:, 3:21*3+3],  # 当前批次的 body_pose
                        left_hand_pose=rec_pose_batch[:, 25*3:40*3],  # 当前批次的 left_hand_pose
                        right_hand_pose=rec_pose_batch[:, 40*3:55*3],  # 当前批次的 right_hand_pose
                        return_verts=True,
                        return_joints=False,
                        leye_pose=tar_pose_batch[:, 69:72],  # 当前批次的 leye_pose
                        reye_pose=tar_pose_batch[:, 72:75],  # 当前批次的 reye_pose
                    )
                    
                    # 存储当前批次的 joints 数据
                    vertices_rec_tensor.append(vertices_rec['vertices'])

                    # 计算 tar_pose 对应的 joints
                    vertices_tar = self.smplx(
                        betas=tar_beta_batch.reshape(-1, 300),  # 当前批次的 betas
                        transl=tar_trans_batch.reshape(-1, 3) - tar_trans_batch.reshape(-1, 3),  # 当前批次的 transl
                        expression=tar_exps_batch.reshape(-1, 100),  # 当前批次的 expressions
                        jaw_pose=tar_pose_batch[:, 66:69],  # 当前批次的 jaw_pose
                        global_orient=tar_pose_batch[:, :3],  # 当前批次的 global_orient
                        body_pose=tar_pose_batch[:, 3:21*3+3],  # 当前批次的 body_pose
                        left_hand_pose=tar_pose_batch[:, 25*3:40*3],  # 当前批次的 left_hand_pose
                        right_hand_pose=tar_pose_batch[:, 40*3:55*3],  # 当前批次的 right_hand_pose
                        return_verts=True,
                        return_joints=False,
                        leye_pose=tar_pose_batch[:, 69:72],  # 当前批次的 leye_pose
                        reye_pose=tar_pose_batch[:, 72:75],  # 当前批次的 reye_pose
                    ...
                    # 存储当前批次的 joints 数据
                    vertices_tar_tensor.append(vertices_tar['vertices'])
                    ...
                    # )
                    # vertices_rec_tensor.append(vertices_rec['joints'])
                    # # torch.cuda.empty_cache()
                    # vertices_tar = self.smplx(
                    #     betas=tar_beta.reshape(bs*n, 300), 
                    #     transl=tar_trans.reshape(bs*n, 3)-tar_trans.reshape(bs*n, 3), 
                    ...
                    #     reye_pose=tar_pose[:, 72:75],
                    # )
                    # vertices_tar_tensor.append(vertices_tar['joints'])
                vertices_rec_joints = torch.cat(vertices_rec_tensor, axis=0)
                vertices_tar_joints = torch.cat(vertices_tar_tensor, axis=0)
                del vertices_rec_tensor, vertices_tar_tensor
                torch.cuda.empty_cache()
                # vertices_rec = self.smplx(
                #     betas=tar_beta.reshape(bs*n, 300), 
                ...
                vectices_loss = self.vectices_loss(vertices_rec_joints, vertices_tar_joints)
                self.tracker.update_meter("ver", "train", vectices_loss.item()*self.args.rec_weight * self.args.rec_ver_weight)
                g_loss_final = g_loss_final + vectices_loss*self.args.rec_weight*self.args.rec_ver_weight
                # torch.cuda.empty_cache()
                vertices_vel_loss = self.vel_loss(vertices_rec_joints[:, 1:] - vertices_rec_joints[:, :-1], vertices_tar_joints[:, 1:] - vertices_tar_joints[:, :-1]) * self.args.rec_weight
                # torch.cuda.empty_cache()
                vertices_acc_loss = self.vel_loss(vertices_rec_joints[:, 2:] + vertices_rec_joints[:, :-2] - 2 * vertices_rec_joints[:, 1:-1], vertices_tar_joints[:, 2:] + vertices_tar_joints[:, :-2] - 2 * vertices_tar_joints[:, 1:-1]) * self.args.rec_weight
                # torch.cuda.empty_cache()
                g_loss_final = g_loss_final + vertices_vel_loss * self.args.rec_weight * self.args.rec_ver_weight
                g_loss_final = g_loss_final + vertices_acc_loss * self.args.rec_weight * self.args.rec_ver_weight 
            ...
            if "VQVAE" in self.args.g_name:
                ...
                g_loss_final = g_loss_final + loss_embedding
                self.tracker.update_meter("com", "train", loss_embedding.item())
                # if loss_embedding > 100:
                #     warnings.warn(f"embedding loss is too large: {loss_embedding.item()}")
