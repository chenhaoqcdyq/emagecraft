        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            if self.args.rec_ver_weight > 0:
                ...
                vertices_rec = self.smplx(
                    ...
                    jaw_pose=rec_pose, 
                    global_orient=torch.zeros(bs*n, 3).to(self.rank), 
                    body_pose=torch.zeros(bs*n, 21*3).to(self.rank), 
                    left_hand_pose=torch.zeros(bs*n, 15*3).to(self.rank), 
                    right_hand_pose=torch.zeros(bs*n, 15*3).to(self.rank), 
                    return_verts=True,
                    # return_joints=True,
                    leye_pose=torch.zeros(bs*n, 3).to(self.rank), 
                    reye_pose=torch.zeros(bs*n, 3).to(self.rank),
                )
