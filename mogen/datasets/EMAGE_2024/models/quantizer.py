...
class EMAQuantizer(nn.Module):
    def __init__(self, n_e, e_dim, beta, decay=0.99, eps=1e-5):
        ...
        self.cluster_size = nn.Parameter(torch.zeros(n_e), requires_grad=False)
        self.embed_avg = nn.Parameter(self.embedding.weight.clone(), requires_grad=False)
    ...
    def forward(self, z):
        ...
        if self.training:
            ...
            self.cluster_size.data.mul_(self.decay).add_(encoding_sum, alpha=1 - self.decay)
