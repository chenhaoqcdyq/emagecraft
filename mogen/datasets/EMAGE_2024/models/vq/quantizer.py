...
class QuantizeEMAReset(nn.Module):
    def __init__(self, nb_code, code_dim, args):
        ...
        self.embedding_proj = nn.Linear(code_dim, code_dim)
        self.reset_codebook()
    ...
    def quantize(self, x, sample_codebook_temp=0.):
        # N X C -> C X N
        quant_codebook= self.embedding_proj(self.codebook)
        # x: NT X C
        # NT X N
        distance = torch.sum(x ** 2, dim=-1, keepdim=True) - \
                   2 * torch.matmul(x, rearrange(quant_codebook, 'n d -> d n')) + \
                   torch.sum(quant_codebook ** 2, dim=1)  # (N * L, b)
    ...
    def dequantize(self, code_idx):
        projected_codebook = self.embedding_proj(self.codebook)
        x = F.embedding(code_idx, projected_codebook)
        return x
    ...
    def forward(self, x, return_idx=False, temperature=0.):
        ...
        if commit_loss.cpu().detach().numpy() > 1000:
            print('commit_loss', commit_loss.cpu().detach().numpy())
        # Passthrough
        x_d = x + (x_d - x).detach()
